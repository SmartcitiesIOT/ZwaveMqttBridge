...
int main( int argc, char* argv[] )
{
	...
	if( !g_initFailed )
	{
		JsonMQTTMessage JsonMessage_o("localhost","1833","192.168.42.37",1833);
		...
		{
			NodeInfo* nodeInfo = *it;
			NodeInfoStructure NodeInfoStructure_m;
			// skip the controller (most likely node 1)
			//if( nodeInfo->m_nodeId == 1) continue;i
			
			Json::StyledStreamWriter writer;
			std::ofstream outFile;
			Json::Value Full_JSON(Json::arrayValue);
			Json::Value Node_i = NodeInfoStructure_m.SetJson(
				to_string(nodeInfo->m_nodeId),
			...
			for( list<ValueID>::iterator it2 = nodeInfo->m_values.begin(); it2 != nodeInfo->m_values.end(); ++it2 )
			{
				...
				ValueStructure ValueStructure_m;
				Json::Value Value_s; 
				Value_s = ValueStructure_m.SetJson(
					Manager::Get()->GetValueLabel(v).c_str(),
					to_string(v.GetType()),
					Manager::Get()->GetValueHelp(v).c_str(),
					Manager::Get()->GetValueUnits(v).c_str(),
					to_string(Manager::Get()->GetValueMin(v)),
					to_string(Manager::Get()->GetValueMax(v))
				);

				Full_JSON.append(Value_s);
				/*
				printf("\t ValueLabel: %s \n", Manager::Get()->GetValueLabel(v).c_str());
				printf("\t\t ValueType: %d \n", v.GetType());
				printf("\t\t ValueHelp: %s \n", Manager::Get()->GetValueHelp(v).c_str());
...
//					Manager::Get()->EnablePoll( v, 2 );		// enables polling with "intensity" of 2, though this is irrelevant with only one value polled
			...
			outFile.open("out1.json");
			writer.write(outFile,Full_JSON);
			outFile.close();	
			JsonMessage_o.Message = Full_JSON;
			JsonMessage_o.MakeJsonString();

			std::cout<<Full_JSON.toStyledString()<<endl;
			std::cout<<JsonMessage_o.JsonString<<endl;
			JsonMessage_o.publish(NULL,"sensors",JsonMessage_o.JsonString.size(),JsonMessage_o.JsonString.c_str());
		}
		pthread_mutex_unlock( &g_criticalSection );
